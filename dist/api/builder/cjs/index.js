 ...
 */
const updateDiffPathInNewFilter = (filterToUpdate, diffPathTagValue, needToUpdateChecksum) => {
    // Make a copy
    let updatedFilter = filterToUpdate.slice();
    updatedFilter = findAndUpdateTag(DIFF_PATH_TAG, diffPathTagValue, updatedFilter);
...
const buildDiff = async (params) => {
    const { oldFilterPath, newFilterPath, patchesPath, name, time, resolution = Resolution.Hours, checksum = false, deleteOlderThanSec = DEFAULT_PATCH_TTL_SECONDS, verbose = false, } = params;
    ...
    let oldFileHasChecksum = false;
    if (parseTag(CHECKSUM_TAG, oldFileSplitted)) {
        oldFileHasChecksum = true;
    }
    ...
    if (calculateChecksumSHA1(oldFileSplitted.join('')) === calculateChecksumSHA1(newFileSplitted.join(''))) {
    ...
    newFileSplitted = updateDiffPathInNewFilter(newFileSplitted, newFilterDiffPathTagValue, oldFileHasChecksum);
